...
km <- survfit(Surv(time, event = 1 - censored) ~ fulltime, data = employment_duration)
km |>
  broom::tidy() |>
...
km |>
  ...
  rename(Treatment_Value = strata, Survives_5_Years = estimate)
```
...
5. Predict the probability of $A_t = 1$ at each time period for every unit. Remember that in R you will need the `type = "response"` option. For grading, what is the predicted probability of $A_0$ for the first unit in the sample?
6.  Define the generalized propensity score $\pi_{it}$ at each time period $t$ for each unit $i$. When $A_{it}=1$, this is the probability of treatment. When $A_{it}=0$, this is the probability of being untreated.
...
1. What is an example of an event history problem in your research area? This could involve a time-to-event outcome or a sequence of events as treatments. How might you use a method from this course to answer that question?
2. Think of a statistical concept you did not know before this quarter that you now understand. What was that concept? Why do you think it particularly stuck with you? Are there ways the course design made that concept easier to learn?
3. Is there a concept from the course that you wish you understood better? Do you have ideas for how we could cover that concept better when this course is taught again?
