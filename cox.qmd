...
Estimate a Cox model for survival time `t` since a heart transplant, using `age` at transplant as a predictor.

- How does the log hazard change for each unit change in `age`?
- What is the hazard ratio for a 1-unit change in `age`?
- Why can't you combine with a Kaplan-Meier estimate of the baseline hazard in this setting?

```{r, echo = F, eval = F}
model <- coxph(
  Surv(t, event = 1 - c) ~ age,
